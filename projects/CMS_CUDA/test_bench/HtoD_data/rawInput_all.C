...
void rawInput_all() {
	...
	inPref = "/home/cms/CMS_CUDA/test_bench/HtoD_data/data_files/";
	outPref = "/home/cms/CMS_CUDA/test_bench/HtoD_data/raw_root_files/";

	for (int i=0; i<5; i++) {
		int bytesI = i;
		while (true) {
			bytes = std::to_string(bytesI);
			inF = "rawData_bytes_" + aList[i] + bytes + ".txt";
			infileName = inPref + inF;
			std::cout << infileName << std::endl;
			ifstream fin;
			fin.open(infileName.c_str());
			if (!fin.fail()) {
				outF = "rawData_bytes_" + aList[i] + bytes + ".root";
